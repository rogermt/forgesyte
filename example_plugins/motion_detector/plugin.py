...
class Plugin:
    ...
    def analyze(self, image_bytes: bytes, options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._previous_frame = alpha * current_frame + (1 - alpha) * self._previous_frame
            ...
            return {
                ...
                "time_since_last_motion": (
                    ...
                    if self._last_motion_time > 0 else None
                ),
                "recent_motion_events": len([
                    e for e in self._motion_history
    ...
    def _gaussian_blur(self, img: np.ndarray, size: int) -> np.ndarray:
        ...
        result = np.apply_along_axis(lambda m: np.convolve(m, kernel, mode='same'), 0, img)
        result = np.apply_along_axis(lambda m: np.convolve(m, kernel, mode='same'), 1, result)
