...
class Plugin:
    ...
    def _get_recommendation(self, is_safe: bool, results: Dict) -> str:
        ...
        flagged = [
            cat["category"] for cat in results["categories"]
            if cat["flagged"]
        ]
        
        if flagged:
            return f"Content flagged for: {', '.join(flagged)}. Manual review recommended."
    ...
    def on_load(self):
        ...
        logger.warning("Using heuristic analysis - replace with real ML model for production")
