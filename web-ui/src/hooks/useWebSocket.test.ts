...
describe("useWebSocket", () => {
    ...
    // -----------------------------------------------------------------------
    // Regression Test: WebSocket Stability (Discussion #345)
    // Ensures WebSocket does not reconnect in a loop after video upload.
    // This test catches the reconnect storm that caused UI freeze.
    // -----------------------------------------------------------------------
    describe("WebSocket Stability (Regression Test - Discussion #345)", () => {
        it("should not trigger reconnect storm when connection drops", async () => {
            const { result } = renderHook(() =>
                useWebSocket({ url: "ws://localhost:8000/v1/stream", plugin: "test" })
            );
            ...
            // Simulate a close (which would increment attempt on reconnect)
            act(() => {
            ...
            // Attempt should be incremented but not spinning
            // The hook starts with attempt 0, increments on reconnect attempt
            expect(result.current.attempt).toBeGreaterThanOrEqual(0);
            expect(result.current.attempt).toBeLessThan(5);
        });

        it("should maintain stable connection when plugin changes", async () => {
            const { result, rerender } = renderHook(
                ({ plugin }: { plugin: string }) =>
                    useWebSocket({ url: "ws://localhost:8000/v1/stream", plugin }),
                { initialProps: { plugin: "plugin-a" } }
            );
            ...
            // Change plugin (simulates tool lock change)
            rerender({ plugin: "plugin-b" });

            // Give time for any potential reconnection storm
            await new Promise((resolve) => setTimeout(resolve, 100));

            // Should not have created excessive reconnection attempts
            expect(result.current.attempt).toBeLessThan(5);
        });
    });
});