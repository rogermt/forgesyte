...
class TaskProcessor:
    ...
    def __init__(
        self, plugin_manager: PluginManager, job_store: JobStore, max_workers: int = 4
    ):
    ...
    async def _process_job(
        self, job_id: str, image_bytes: bytes, plugin_name: str, options: dict
    ):
        ...
        try:
            ...
            await self.job_store.update(
                ...
                result={**result, "processing_time_ms": processing_time},
    ...
    async def cancel_job(self, job_id: str) -> bool:
        ...
        if job and job["status"] == JobStatus.QUEUED:
            await self.job_store.update(
                job_id, status=JobStatus.ERROR, error="Cancelled by user"
            )
            return True
        return False
