...
@pytest.mark.asyncio
async def test_lifespan_worker_thread_with_env_enabled(mock_plugin_manager):
    """Test that lifespan starts JobWorker thread when FORGESYTE_ENABLE_WORKERS=1.

    v0.15.0: Ray initialization removed - JobWorker runs as daemon thread.
    """
    with (
        patch("app.main.PluginRegistry", return_value=mock_plugin_manager),
        ...
        patch.dict(
            os.environ,
            {"FORGESYTE_ENABLE_WORKERS": "1"},
            clear=False,
        ),
    ):
        # Mock the thread to prevent real worker from starting
